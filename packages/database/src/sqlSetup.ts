    ...
    try {
        ...
        await client.query(`
            CREATE TABLE IF NOT EXISTS bot.tbl_snippets (
                snippet_id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
                discord_id BIGINT REFERENCES bot.tbl_user(discord_id) ON DELETE CASCADE ON UPDATE CASCADE,
                snippet_code TEXT NOT NULL,
                snippet_lang VARCHAR(10) NOT NULL,
                snippet_outcome TEXT NOT NULL
            )
        `);
...
export async function seedDatabase(pool: pg.Pool) {
    ...
    try {
        ...
        `);
        await client.query(`
            INSERT INTO bot.tbl_snippets (snippet_ID, DISCORD_ID, snippet_CODE, snippet_LANG, snippet_OUTCOME)
            VALUES 
